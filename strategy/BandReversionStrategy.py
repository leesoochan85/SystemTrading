...
class BandReversionStrategy(QThread):
    ...
    def __init__(self, kiwoom, auto_init=True):
        super().__init__()
        self.strategy_name = "BandReversionStrategy"
        self.kiwoom = kiwoom
        self.universe = {}
        self.deposit = 0
    ...
    def init_strategy(self):
        try:
            ...
            send_message("[BandReversionStrategy] 초기화 완료")
        except Exception:
    ...
    def set_universe_real_time(self):
        ...
        for i in range(0, len(codes), chunk_size):
            screen_no = str(1100 + (i // chunk_size))
    ...
    def get_bollinger_df(self, code, mfi_period=14):
        ...
        # 기존 반전 전략 계산식 유지
        pb_denominator = df["upper"] - df["mid20"]
        ...
        df["intraday_intensity"] = np.where(
            hl_diff != 0,
            ((2 * df["close"] - df["high"] - df["low"]) / hl_diff) * df["volume"],
            0,
        )
        ii_sum = df["intraday_intensity"].rolling(window=21).sum()
        vol_sum = df["volume"].rolling(window=21).sum()
        df["intraday_intensity_percent"] = np.where(vol_sum != 0, (ii_sum / vol_sum) * 100, np.nan)
        return df
    ...
    def check_sell_signal(self, code):
        ...
        latest = df.iloc[-1]
        pb = latest["percent_b"]
        iip = latest["intraday_intensity_percent"]
        if pd.isna(pb) or pd.isna(iip):
            return False

        # 기존 반등 청산 조건은 그대로 유지한다.
        return bool(pb > 0.95 and iip < 0)
    ...
    def order_sell(self, code):
        ...
        result = self.kiwoom.send_order(
            "send_sell_order", "2002", 2, code, quantity,
    ...
    # def run(self):
    #     print("[BandReversionStrategy] run 시작")
